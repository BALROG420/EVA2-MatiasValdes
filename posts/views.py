...
@staff_member_required(login_url='posts:login')
...
def editar_banda(request,banda_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('posts:banda_detalle', banda_id=banda.id)
    else:
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render(request, 'posts/login.html', {'error': 'Usuario o contraseña incorrectos'})
